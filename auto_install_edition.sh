...
if [ "$#" -eq "0" ];then
  if [ -f ${PWD}/ssh/id_rsa ];then
    ssh_private_key_file=${PWD}/ssh/id_rsa
  else
    echo "没有找到 ssh private key file，请指定您的私钥"
    exit 1
  fi
else
...
cd ${WorkDir}
ssh-agent sh -c "ssh-add ${ssh_private_key_file}; git clone git@gitee.com:wenvki/idcops-edition.git django-idcops"
cd ${ProjDir}
